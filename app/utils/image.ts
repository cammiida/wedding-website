...
const buildCloudinaryUrl = (src: string, options: ImageOptions) => {
  ...
  if (options.mode === "face") {
    return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/fetch/f_auto,c_thumb,w_300,h_300,g_face/${encodeURIComponent(
      src
    )}`;
  }
  ...
  if (options.mode === "portrait") {
    return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/fetch/f_auto,c_fill,w_600,h_900,g_face/${encodeURIComponent(
      src
    )}`;
  }
  ...
  if (options.mode === "landscape") {
    return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/fetch/f_auto,c_fill,w_1200,h_800,g_face/${encodeURIComponent(
      src
    )}`;
  }
